...
resolved:
	...
	*rec_offs_base(offsets) = static_cast<rem0offset_t>(rec - (lens + 1))
...
resolved:
		...
		*rec_offs_base(offsets) = static_cast<rem0offset_t>(
			(rec - (lens + 1)) | REC_OFFS_COMPACT);
	} else {
		/* Old-style record: determine extra size and end offsets */
		offs = REC_N_OLD_EXTRA_BYTES;
		if (rec_get_1byte_offs_flag(rec)) {
			offs += static_cast<rem0offset_t>(
	...
	mem_heap_t**		heap)	/*!< in/out: memory heap */
{
	...
	if (UNIV_UNLIKELY(!offsets)
		...
		offsets = static_cast<rem0offset_t*>(
					...
					offsets[0] allocated elements */
{
	...
	do {
		...
		if (UNIV_UNLIKELY(!field->fixed_len)) {
			...
			len = offs += static_cast<rem0offset_t>(field->fixed_len);
		}
resolved:
		rec_offs_base(offsets)[i + 1] = len;
	} while (++i < rec_offs_n_fields(offsets));
	...
	*rec_offs_base(offsets)
		= static_cast<rem0offset_t>((lens - extra + REC_N_NEW_EXTRA_BYTES)
					| REC_OFFS_COMPACT | any_ext);
}
...
{
	mem_heap_t*	heap	= NULL;
	rem0offset_t*	offsets	= rec_get_offsets(
	...
	const dict_index_t*	index)
{
	...
	ulint		len;
	mem_heap_t*	heap		= NULL;
	rem0offset_t offsets_[REC_OFFS_HEADER_SIZE + MAX_REF_PARTS + 2];
	rec_offs_init(offsets_);
	rem0offset_t* offsets = offsets_;
