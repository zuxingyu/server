...
/*===============================*/
	...
	page_zip_des_t*	page_zip,/*!< in: compressed page whose uncompressed
				part will be updated, or NULL */
	const rem0offset_t*	offsets,/*!< in: rec_get_offsets(rec, index) */
				...
				(0 if none) */
{
	...
	rem0offset_t	offsets_[REC_OFFS_NORMAL_SIZE];
	rem0offset_t*	offsets		= offsets_;
	rem0offset_t	offsets2_[REC_OFFS_NORMAL_SIZE];
	rem0offset_t*	offsets2	= offsets2_;
...
/*=======================*/
	...
	rem0offset_t**	offsets,/*!< out: offsets on *rec */
...
/*======================*/
	...
	const rem0offset_t*	offsets,/*!< in: rec_get_offsets() on cursor */
	const upd_t*	update,	/*!< in: update vector */
...
#ifdef UNIV_DEBUG
	rem0offset_t*	offsets,/*!< in/out: offsets of the cursor record */
#endif /* UNIV_DEBUG */
...
/*====================*/
	...
	btr_cur_t*	cursor,	/*!< in: cursor on the record to update;
				...
				same record */
	rem0offset_t*	offsets,/*!< in/out: offsets on cursor->page_cur.rec */
	const upd_t*	update,	/*!< in: update vector */
	ulint		cmpl_info,/*!< in: compiler info on secondary index
				updates */
	que_thr_t*	thr,	/*!< in: query thread */
...
/*======================*/
	...
	btr_cur_t*	cursor,	/*!< in: cursor on the record to update;
				cursor stays valid and positioned on the
				same record */
	rem0offset_t**	offsets,/*!< out: offsets on cursor->page_cur.rec */
	...
	dict_index_t*	index)	/*!< in: index corresponding to page */
{
	...
	if (page) {
		...
		rem0offset_t offsets[REC_OFFS_HEADER_SIZE + MAX_REF_PARTS + 2];
	...
	dict_index_t*	index)	/*!< in: index */
{
	...
	mtr_t		mtr;
	mem_heap_t*	heap		= NULL;
	rem0offset_t*	offsets_rec	= NULL;
	rem0offset_t*	offsets_next_rec = NULL;
	...
	for (i = 0; i < n_sample_pages; i++) {
		...
		while (!page_rec_is_supremum(rec)) {
			...
			offsets_next_rec. */
			{
				rem0offset_t* offsets_tmp = offsets_rec;
				offsets_rec = offsets_next_rec;
	...
	void check()
	{
		...
		*m_rec		= btr_pcur_get_rec(m_pcur);
		...
		ut_d(rec_offs_make_valid(
			*m_rec, index, const_cast<rem0offset_t*>(m_offsets)));
...
/*============================*/
	btr_pcur_t*	pcur,		/*!< in/out: a persistent cursor. if
					...
					be repositioned. */
	rem0offset_t*	offsets,	/*!< in/out: rec_get_offsets() on
