	...
	mtr_t*		mtr)		/*!< in/out: mini-transaction */
{
	...
	rem0offset_t*	rec_offsets;
	rem0offset_t*	prev_rec_offsets;
	ulint		i;
	...
	rec_offsets = static_cast<rem0offset_t*>(
		mem_heap_alloc(heap, i * sizeof *rec_offsets));
	prev_rec_offsets = static_cast<rem0offset_t*>(
...
{
	rem0offset_t*	offsets_rec		= offsets1;
	rem0offset_t*	offsets_next_rec	= offsets2;
	const rec_t*	rec;
	...
	ib_uint64_t*		n_external_pages)
{
	...
	offsets1 = static_cast<rem0offset_t*>(mem_heap_alloc(
			heap, size * sizeof *offsets1));

	offsets2 = static_cast<rem0offset_t*>(mem_heap_alloc(
