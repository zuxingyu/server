...
static const rem0offset_t REC_1BYTE_SQL_NULL_MASK= 0x80;
/** SQL null flag in a 2-byte offset of ROW_FORMAT=REDUNDANT records */
static const rem0offset_t REC_2BYTE_SQL_NULL_MASK= 0x8000;

/** In a 2-byte offset of ROW_FORMAT=REDUNDANT records, the second most
significant bit denotes that the tail of a field is stored off-page. */
static const rem0offset_t REC_2BYTE_EXTERN_MASK= 0x4000;

static const size_t RECORD_OFFSET= 2;
static const size_t INDEX_OFFSET=
    RECORD_OFFSET + sizeof(rec_t *) / sizeof(rem0offset_t);

/* Length of the rec_get_offsets() header */
static const size_t REC_OFFS_HEADER_SIZE=
#ifdef UNIV_DEBUG
    sizeof(rec_t *) / sizeof(rem0offset_t) +
    sizeof(dict_index_t *) / sizeof(rem0offset_t) +
#endif /* UNIV_DEBUG */
    2;
...
static const rem0offset_t DATA_MASK= 0x3fff;
/** 2 upper bits */
static const rem0offset_t TYPE_MASK= ~DATA_MASK;
inline field_type_t get_type(rem0offset_t n)
{
  return static_cast<field_type_t>(n & TYPE_MASK);
}
inline void set_type(rem0offset_t &n, field_type_t type)
{
  n= (n & DATA_MASK) | static_cast<rem0offset_t>(type);
}
inline rem0offset_t get_value(rem0offset_t n) { return n & DATA_MASK; }
inline rem0offset_t combine(rem0offset_t value, field_type_t type)
{
  return get_value(value) | static_cast<rem0offset_t>(type);
}

/** Compact flag ORed to the extra size returned by rec_offs_base()[0] */
static const rem0offset_t REC_OFFS_COMPACT= 1 << 15;
/** External flag in offsets returned by rec_offs_base()[0] */
static const rem0offset_t REC_OFFS_EXTERNAL= 1 << 14;
...
/*====================*/
	...
	ulint			node_ptr,/*!< in: nonzero=node pointer,
					0=leaf node */
	rem0offset_t*		offsets)/*!< in/out: array consisting of
...
/*==============*/
	...
	const rem0offset_t*		offsets)/*!< in: array returned by
					rec_get_offsets() */
...
/*================*/
	const rec_t*		rec,	/*!< in: record */
	const dict_index_t*	index,	/*!< in: record descriptor */
	rem0offset_t*		offsets)/*!< in: array returned by
					rec_get_offsets() */
	MY_ATTRIBUTE((nonnull));
#else
...
/*===================*/
	const rem0offset_t*	offsets,/*!< in: array returned by rec_get_offsets() */
	ulint		n,	/*!< in: index of the field */
	ulint*		len)	/*!< out: length of the field; UNIV_SQL_NULL
				if SQL null */
	MY_ATTRIBUTE((nonnull));
...
/*=====================*/
	const rec_t*	rec,		/*!< in: record */
	const rem0offset_t*	offsets)	/*!< in: rec_get_offsets(rec) */
	MY_ATTRIBUTE((warn_unused_result));
/******************************************************//**
...
/*=================*/
	const rem0offset_t*	offsets)/*!< in: array for rec_get_offsets() */
	MY_ATTRIBUTE((warn_unused_result));
/**********************************************************//**
...
/*=================*/
	rem0offset_t*offsets,	/*!< out: array for rec_get_offsets(),
				must be allocated */
...
/*==================*/
	...
	const dict_index_t*	index,	/*!< in: record descriptor */
	rem0offset_t*		offsets)/*!< in/out: array of offsets;
...
{
	/** Constructor */
	rec_offsets_print(const rec_t* rec, const rem0offset_t* offsets) :
...
public:
	...
	rec_printer(const rec_t* rec, const rem0offset_t* offsets)
		:
		std::ostringstream ()
	{
		rec_print(*this, rec,
	...
	@param offsets rec_get_offsets(rec, ...) */
	rec_printer(const rec_t* rec, ulint info, const rem0offset_t* offsets)
		:
		std::ostringstream ()
	{
