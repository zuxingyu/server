...
public:
	...
	bool remove(
		...
		rem0offset_t*		offsets) UNIV_NOTHROW
	{
...
private:
	...
	dberr_t	adjust_cluster_index_blob_columns(
		rec_t*		rec,
		const rem0offset_t*	offsets) UNIV_NOTHROW;
	...
	bool	purge(const rem0offset_t* offsets) UNIV_NOTHROW;
...
private:
	...
	/** Record offset */
	rem0offset_t		m_offsets_[REC_OFFS_NORMAL_SIZE];

	/** Pointer to m_offsets_ */
	rem0offset_t*		m_offsets;
...
inline
bool
PageConverter::purge(const rem0offset_t* offsets) UNIV_NOTHROW
{
	...
	const dict_table_t*	table)		/*!< in: table to import */
{
	...
	if (!page_rec_is_infimum(rec)) {
		...
		rem0offset_t	offsets_[1 + REC_OFFS_HEADER_SIZE];
