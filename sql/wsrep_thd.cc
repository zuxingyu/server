...
static int wsrep_kill(wsrep_kill_t* item)
{
  ...
  // Note that we need to release LOCK_thd_data and
  // LOCK_thd_kill mutex from BF thread to obey safe
  // mutex ordering of LOCK_thread_count -> LOCK_thd_kill
  // that both are taken on find_thread_by_id
  mysql_mutex_unlock(&bf_thd->LOCK_thd_data);
