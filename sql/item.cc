...
bool Item_field::fix_fields(THD *thd, Item **reference)
{
  ...
  if (!field)					// If field is not checked
  {
    ...
    */
    ...
    /* Detect non-versioned DML if system field is used explicitly in query. */
    if (from_field != view_ref_found && from_field->vers_sys_field() &&
        origin != Item::ORIGIN_VERS_COND && thd->vers_modify_history())
    {
