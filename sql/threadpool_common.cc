...
retry:
    switch (threadpool_process_request(thd))
    {
      case DISPATCH_COMMAND_WOULDBLOCK:
        ...
        {
          /*
          All async operations finished meanwhile. Nobody is going to wake up this THD,
          thus need to retry process_request(), as-if THD was resumed.
          */
...
resume:
    switch ((retval = do_command(thd, false)))
    {
      case DISPATCH_COMMAND_WOULDBLOCK:
      case DISPATCH_COMMAND_ERROR:
        goto end;
