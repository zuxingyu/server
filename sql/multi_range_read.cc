                                     ...
                                     Cost_estimate *cost)
{
  ...
  */
  ulonglong prev_range_last_block= UNUSED_PAGE_RANGE;
  ...
  *bufsz= 0;
  ...
  while (!seq->next(seq_it, &range))
  {
    ...
    if ((range.range_flag & UNIQUE_RANGE) && !(range.range_flag & NULL_RANGE))
    {
      rows= 1;
      /*
        In this case we do not call records_records_in_range() and as a result
        do not get any info on the edge blocks for this range. However if it
        happens the range for which we have such info uses the same block for
        the its first record as the last range for which such info is provided
        uses for its last record then this range can be assigned later to one
        of the blocks used by other ranges.

        Note that we don't have to increment edge_blocks_cnt or range_blocks_cnt
        here.
      */
      single_point_ranges++;
    }
             ...
             table->key_info[keyno].actual_rec_per_key(keyparts_used - 1) > 0.5)
    {
      ...
      edge_blocks_cnt+= ((MY_MAX(rows, 1) - 1) / avg_block_records + 1);
    }
    else
    {
      ...
      if (pages.first_page == UNUSED_PAGE_RANGE)
      {
        /*
	  ...
	  */
          ...
          else
	  {
            /*
              ...
              We don't know where the first key was positioned in the block,
              so we assume it was half in into the block as average.
