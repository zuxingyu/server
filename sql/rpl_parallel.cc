...
public:
  uint32 worker_idx;
  Atomic_counter<int32_t> cnt; // number of namesake consecutive xa:s at execution
};
  ...
  */
  ...
  if (!rgi->current_xid.is_null())
  {
    ...
    if (el)
    {
      ...
      lf_hash_search_unpin(pins); // TODO/fixme: unpin after delete is fine?
    }
    else
    {
...
/*
  ...
  For the PREPARED one, a record consisting of the xid and an associated worker
  is inserted into  a local rli parallel xid hash.
  ...
  if (!reuse)
  {
        ...
        (Gtid_log_event::FL_COMPLETED_XA | Gtid_log_event::FL_PREPARED_XA))
    {
      ...
      else
      {
        ...
        // XA completion event may arrive without a previous prepare one.
...
bool rpl_parallel::xid_cache_insert(XID *xid, uint32 idx)
{
  ...
  if (!res)
  {
    // TODO/FIXME: should I pin-search the inserted record for further working it up?
    static_cast<XID_cache_element_para*>(new_element.xid_cache_element)->
      worker_idx= idx;
    static_cast<XID_cache_element_para*>(new_element.xid_cache_element)->cnt= 1;
  }
  return res;
}
